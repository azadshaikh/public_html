<x-app-layout title="Edit Tag: {{ $tag->title }}">
    {{-- Page Header --}}
    <x-page-header title="Edit Tag"
    ...
    <form data-dirty-form class="needs-validation" action="{{ route('cms.tags.update', $tag) }}" method="POST" id="tag-form" novalidate>
        @csrf
        @method('PUT')
    ...
    @can('delete_tags')
        <form id="trash-tag-form" method="POST" action="{{ route('cms.tags.destroy', $tag->id) }}" class="d-none">
            @csrf
            @method('DELETE')
        </form>
    @endcan
