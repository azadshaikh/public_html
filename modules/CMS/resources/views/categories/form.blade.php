...
<div class="row">
    ...
    <div class="col-md-4 mb-3">
        @php
            ...
            $statusValue = old('status', $isEdit ? $category->status : ($defaults['status'] ?? 'draft'));
            $currentSlug = old('slug', isset($category) ? $category->slug : '');
            $baseUrl = rtrim(config('app.url'), '/');
            $actualPermalink = $isEdit ? url($category->permalink_url) : null;
        ...
        <div class="card mb-3">
            ...
            <div class="card-body py-3">
                <x-media-picker.image-field class="form-group" id="feature_image"
                    name="feature_image"
                    :value="old('feature_image', isset($category) ? $category->feature_image_id : '')"
                    :valueUrl="isset($category) ? get_media_url($category->feature_image_id) : null"
                    :previewUrl="isset($category) ? get_media_url($category->feature_image_id) : null"
            ...
            <div class="card-body py-3">
                ...
                <div class="mb-0">
                    <x-form-elements.select class="form-group" id="parent_id" name="parent_id"
                        ...
                        label="Parent Category" labelclass="form-label" placeholder="Select parent category (optional)"
                        :options="json_encode($parentCategoryOptions ?? [], true)" />
                </div>
            </div>
        </div>
                ...
                <div class="card-body py-3">
                    ...
                    <div class="mb-3">
                        <x-form-elements.select class="form-group" id="template" name="template"
                            :value="old('template', isset($category) ? $category->template : '')"
                            label="Template" labelclass="form-label"
                            :options="json_encode($templateOptions ?? [], true)" />
                        <div class="form-text">Choose a different layout for this category.</div>
                    </div>
                    @endif
                    ...
                    @if (isset($category) && $category->exists)
                        <div class="mt-3 pt-3 border-top">
                            <span class="fw-semibold fs-6 align-items-center d-flex cursor-pointer text-gray-700"
                                ...
                                <i class="ri-history-line me-1"></i> {{ $category->revisions_count }} Revisions
                            </span>
                        </div>
                    @endif
                </div>
        ...
        <div class="d-grid mb-3">
            <button class="btn btn-primary btn-lg" type="submit" id="save-btn">
                ...
                <span class="btn-text">
                    {{ isset($category) && $category->exists ? 'Save Changes' : 'Create Category' }}
                </span>
            </button>
        </div>
                    ...
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                                ...
                                <p class="text-muted small mb-0">Move this category to trash.</p>
                            </div>
...
@if (isset($category) && !empty($category))
    <x-revisions-modal :modeltitle="$category->title" :revisions="$category->revisionHistory" :preview="$category->type" />
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p class="mb-0">Are you sure you want to move <strong>"{{ $category->title }}"</strong> to trash?</p>
                        <p class="text-muted small mb-0 mt-2">You can restore this category later from the trash.</p>
                    </div>
                    <div class="modal-footer border-0 pt-0">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="button" class="btn btn-danger" onclick="document.getElementById('trash-category-form')?.requestSubmit();">
