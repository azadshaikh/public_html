...
<div class="row">
    ...
    <div class="col-md-4 mb-3">
        @php
            ...
            $statusValue = old('status', $isEdit ? $page->status : ($defaults['status'] ?? 'draft'));
            $visibilityValue = old('visibility', isset($page) ? $page->visibility : 'public');
            $currentSlug = old('slug', isset($page) ? $page->slug : '');
            $baseUrl = rtrim(config('app.url'), '/');
            $actualPermalink = $isEdit ? url($page->permalink_url) : null;

            $publishedAtFormatted = null;
            if (old('published_at')) {
                $publishedAtFormatted = old('published_at');
            } elseif (isset($page) && $page->published_at) {
                $publishedAtFormatted = app_date_time_format($page->published_at, 'datetime');
            }

            $showPasswordFields = $visibilityValue === 'password';
            $hasExistingPassword = isset($page) && $page->isPasswordProtected();
        ...
        <div class="card mb-3">
            ...
            <div class="card-body py-3">
                <x-media-picker.image-field class="form-group" id="feature_image"
                    name="feature_image"
                    :value="old('feature_image', isset($page) ? $page->feature_image_id : '')"
                    :valueUrl="isset($page) ? get_media_url($page->feature_image_id) : null"
                    :previewUrl="isset($page) ? get_media_url($page->feature_image_id) : null"
        ...
        <div class="card mb-3">
            ...
            <div class="card-body py-3">
                ...
                {{-- Parent Page --}}
                <div class="mb-0">
                    <x-form-elements.select class="form-group" id="parent_id" name="parent_id"
                        value="{{ old('parent_id', isset($page) ? $page->parent_id : '') }}"
                        label="Parent Page" labelclass="form-label" placeholder="Select parent page (optional)"
                        :options="json_encode($parentPageOptions ?? [], true)" />
                </div>
            </div>
        </div>
            ...
            <div class="collapse {{ $hasAnyError ? 'show' : '' }}" id="more-options-collapse">
                <div class="card-body py-3">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">Required for scheduled pages.</div>
                    </div>
                    ...
                    <div class="mb-3">
                        <x-form-elements.select class="form-group" id="template" name="template"
                            :value="old('template', isset($page) ? $page->template : '')"
                            label="Template" labelclass="form-label"
                            :options="json_encode($templateOptions ?? [], true)" />
                        <div class="form-text">Choose a different layout for this page.</div>
                    </div>
                    @endif
                    ...
                    <div id="password-hint-container" style="{{ $showPasswordFields ? '' : 'display: none;' }}">
                        <x-form-elements.input
                            ...
                            id="password_hint"
                            name="password_hint"
                            :value="old('password_hint', isset($page) ? $page->password_hint : '')"
                    ...
                    @if (isset($page) && $page->exists)
                        <div class="mt-3 pt-3 border-top">
                            <span class="fw-semibold fs-6 align-items-center d-flex cursor-pointer text-gray-700"
                                ...
                                <i class="ri-history-line me-1"></i> {{ $page->revisions_count }} Revisions
                            </span>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                                ...
                                <p class="text-muted small mb-0">Move this page to trash.</p>
                            </div>
...
@if (isset($page) && !empty($page))
    <x-revisions-modal :modeltitle="$page->title" :revisions="$page->revisionHistory" :preview="$page->type" />
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p class="mb-0">Are you sure you want to move <strong>"{{ $page->title }}"</strong> to trash?</p>
                        <p class="text-muted small mb-0 mt-2">You can restore this page later from the trash.</p>
                    </div>
                    <div class="modal-footer border-0 pt-0">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="button" class="btn btn-danger" onclick="document.getElementById('trash-page-form')?.requestSubmit();">
