...
import type { AvailableWidget, WidgetArea, WidgetIndexPageProps, WidgetInstance } from '../../../types/cms';
...
function WidgetPreviewItem({
    ...
    return (
        <li className="flex items-center gap-2 py-1.5 text-sm">
            ...
            <span className="min-w-0 flex-1 truncate font-medium">{widget.title || 'Untitled Widget'}</span>
            ...
            <CardContent className="flex-1 py-0">
                {count > 0 ? (
                    <ul className="divide-y">
                        {preview.map((w) => (
                            <WidgetPreviewItem key={w.id} widget={w} availableWidgets={availableWidgets} />
                        ))}
                        {overflow > 0 && (
                    ...
                    <p className="py-2 text-sm text-muted-foreground">No widgets in this area.</p>
                )}
            </CardContent>

            <CardFooter className="pt-4">
                <Button asChild className="w-full" variant="outline">
                    <Link href={route('cms.appearance.widgets.edit', { area_id: area.id })}>
...
export default function WidgetsIndex({ widgetAreas, currentWidgets, availableWidgets }: WidgetIndexPageProps) {
    return (
        <AppLayout
        ...
        >
            {widgetAreas.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
                    ...
                    <h3 className="text-lg font-medium">No widget areas available</h3>
                    <p className="max-w-sm text-sm text-muted-foreground">
                        Your active theme doesn't define any widget areas, or no theme is currently active.
                    </p>
                </div>
            ) : (
