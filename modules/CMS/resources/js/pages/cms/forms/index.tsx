...
export default function FormsIndex({
    ...
    const handleBulkAction = (
        action: string,
        selected: FormListItem[],
        clearSelection: () => void,
    ) => {
        if (selected.length === 0) return;
        router.post(
            route('cms.form.bulk-action'),
            { action, ids: selected.map((f) => f.id), status: currentStatus },
    ...
    const columns: DatagridColumn<FormListItem>[] = [
        ...
        },
        {
            ...
            cellClassName:
                'w-28 text-right tabular-nums text-sm text-muted-foreground',
            sortable: true,
        },
        {
            key: 'conversion_rate_display',
            header: 'Conversion',
            headerClassName: 'w-28 text-right',
            cellClassName:
                'w-28 text-right tabular-nums text-sm text-muted-foreground',
                    ...
                    ? [
                          {
                              label: 'Restore',
                              icon: <RefreshCwIcon />,
                              href: route('cms.form.restore', form.id),
                              method: 'PATCH' as const,
                              confirm: `Restore "${form.title}"?`,
                          },
                      ]
                    : []),
                ...(canDeleteForms
                    ? [
                          {
                              ...
                              icon: <Trash2Icon />,
                              href: route('cms.form.force-delete', form.id),
                              method: 'DELETE' as const,
                              confirm: `⚠️ Permanently delete "${form.title}"?`,
        ...
        return [
            ...
            ...(canDeleteForms
                ? [
                      {
                          label: 'Move to Trash',
                          href: route('cms.form.destroy', form.id),
                          method: 'DELETE' as const,
                          confirm: `Move "${form.title}" to trash?`,
                          icon: <Trash2Icon />,
                          variant: 'destructive' as const,
                      },
                  ]
            ...
            ? [
                  {
                      ...
                      icon: <Trash2Icon />,
                      variant: 'destructive' as const,
                      confirm: 'Move selected forms to trash?',
                      onSelect: (items: FormListItem[], clear: () => void) =>
            ...
            ? [
                  {
                      ...
                      label: 'Restore',
                      icon: <RefreshCwIcon />,
                      confirm: 'Restore selected forms?',
                      onSelect: (items: FormListItem[], clear: () => void) =>
            ...
            ? [
                  {
                      ...
                      icon: <Trash2Icon />,
                      variant: 'destructive' as const,
                      confirm: '⚠️ Permanently delete selected forms?',
                      onSelect: (items: FormListItem[], clear: () => void) =>
    ...
    return (
        <AppLayout
            ...
            title="Forms"
            description="Manage contact and submission forms"
            headerActions={
                canAddForms ? (
                    <Button asChild>
                        <Link href={route('cms.form.create')}>
            ...
            <div className="flex flex-col gap-6">
                <Datagrid
                    ...
                    empty={{
                        ...
                        description:
                            'Try a different filter or create the first form.',
                    }}
                />
            </div>
        </AppLayout>
