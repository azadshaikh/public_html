...
import type { BillingOption, RefundFormValues } from '../../types/billing';
...
export default function RefundForm({
    ...
    const form = useAppForm<RefundFormValues>({
        defaults: initialValues,
        rememberKey:
            mode === 'create'
                ? 'billing.refunds.create'
                : `billing.refunds.edit.${refund?.id}`,
        dirtyGuard: { enabled: true },
        rules: {
            payment_id: [formValidators.required('Payment')],
    ...
    const submitUrl =
        mode === 'create'
            ? route('app.billing.refunds.store')
            : route('app.billing.refunds.update', refund!.id);
    const submitLabel = mode === 'create' ? 'Create Refund' : 'Update Refund';
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <FieldGroup cols={2}>
                                <Field>
                                    ...
                                    <NativeSelect
                                        ...
                                        onChange={(e) => form.setData('payment_id', e.target.value)}
                                    >
                                        <NativeSelectOption value="">Select payment</NativeSelectOption>
                                    ...
                                    <FieldError>{form.errors.payment_id}</FieldError>
                                </Field>
                                <Field>
                            ...
                            <FieldGroup cols={2}>
                                ...
                                <Field>
                                    <FieldLabel>Gateway Refund ID</FieldLabel>
                                    <Input
                                        value={form.data.gateway_refund_id}
                                        onChange={(e) => form.setData('gateway_refund_id', e.target.value)}
                                    />
                                </Field>
                            </FieldGroup>
                            <FieldGroup cols={2}>
                                <Field>
                                    <FieldLabel>Refund Number</FieldLabel>
                                    <Input
                                        value={form.data.refund_number}
                                        onChange={(e) => form.setData('refund_number', e.target.value)}
                            ...
                            <FieldGroup cols={2}>
                                <Field>
                                    <FieldLabel>Refunded At</FieldLabel>
                                    <Input
                                        type="date"
                                        value={form.data.refunded_at}
                                        onChange={(e) => form.setData('refunded_at', e.target.value)}
                                    />
                                </Field>
                                <Field>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => form.setData('failed_at', e.target.value)}
                                    />
                                </Field>
                            </FieldGroup>
                            <Field>
