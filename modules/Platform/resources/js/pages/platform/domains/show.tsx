...
import { ActivityIcon, GlobeIcon, PencilIcon, PlusIcon, ShieldCheckIcon, SparklesIcon } from 'lucide-react';
...
export default function DomainsShow({ domain, dnsRecords, sslCertificates, activities }: DomainsShowPageProps) {
            ...
            <div className="flex flex-col gap-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Core ownership, registrar, and DNS lifecycle details.</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        <MetricCard label="Domain" value={domain.name} />
                        <MetricCard label="Type" value={domain.type_label} />
                        <MetricCard label="Status" value={domain.status_label} />
                        <MetricCard label="Agency" value={domain.agency_name} />
                        <MetricCard label="Registrar" value={domain.registrar_name} />
                        <MetricCard label="DNS provider" value={domain.dns_provider} />
                        <MetricCard label="DNS zone" value={domain.dns_zone_id} />
                        <MetricCard label="Registered" value={domain.registered_date} />
                        <MetricCard label="Expires" value={domain.expires_date} />
                        <MetricCard label="Updated" value={domain.updated_date} />
                        <MetricCard label="DNS records" value={domain.dns_records_count} />
                        <MetricCard label="Certificates" value={domain.ssl_certificates_count} />
                        <MetricCard label="Latest certificate expiry" value={domain.latest_certificate_expires_at} />
                        <MetricCard label="Created" value={domain.created_at} />
                        <MetricCard label="Last changed" value={domain.updated_at} />
                    </CardContent>
                </Card>
                ...
                <div className="grid gap-6 xl:grid-cols-2">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Certificates currently attached to this domain.</CardDescription>
                        </CardHeader>
                        <CardContent className="flex flex-col gap-3">
                            {sslCertificates.length === 0 ? (
                                <p className="text-sm text-muted-foreground">No SSL certificates have been attached yet.</p>
                            ) : (
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Current WHOIS name server values stored for this domain.</CardDescription>
                        </CardHeader>
                        <CardContent className="flex flex-col gap-3">
                            {domain.name_servers.length === 0 ? (
                                <p className="text-sm text-muted-foreground">No name servers are recorded yet.</p>
                            ) : (
                                domain.name_servers.map((nameServer) => (
                                    <div key={nameServer} className="rounded-lg border p-3 text-sm font-medium text-foreground">
                                        {nameServer}
                                    </div>
                                ))
                            )}
                ...
                <div className="grid gap-6 xl:grid-cols-2">
                            ...
                            {dnsRecords.length === 0 ? (
                                ...
                                dnsRecords.map((record) => (
                                    <div key={record.id} className="rounded-lg border p-3">
                                        <div className="flex items-center justify-between gap-3">
                                            <p className="font-medium text-foreground">{record.type} · {record.name}</p>
                                            <span className="text-xs uppercase tracking-wide text-muted-foreground">
                                                TTL {record.ttl ?? 'auto'}
                                            </span>
                                        </div>
                                        <p className="mt-1 text-sm text-muted-foreground">{record.value}</p>
                                        {record.disabled ? <p className="mt-2 text-xs text-muted-foreground">Disabled</p> : null}
                                    </div>
                                ))
                            )}
                        </CardContent>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Most recent WHOIS refreshes, metadata changes, and certificate updates.</CardDescription>
                        </CardHeader>
                        <CardContent className="flex flex-col gap-3">
                            {activities.length === 0 ? (
