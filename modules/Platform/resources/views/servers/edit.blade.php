{{-- Edit Server --}}
<x-app-layout title="Edit Server">

    @php
        $actions = [];

        if (Route::has('platform.servers.show')) {
            $actions[] = [
                'label' => 'Show',
    ...
    <x-page-header title="Edit Server"
        description="Update server details" layout="form"
        :actions="$actions"
        :breadcrumbs="[
            ['label' => 'Dashboard', 'href' => route('dashboard')],
            ['label' => 'Platform'],
            ['label' => 'Servers', 'href' => route('platform.servers.index')],
            ['label' => '#' . $server->id, 'href' => route('platform.servers.show', $server->id)],
    ...
    <form data-dirty-form class="needs-validation" id="server-form" method="POST" action="{{ $formConfig['action'] ?? route('platform.servers.update', $server->id) }}" novalidate>
        @csrf
        @method('PUT')
