  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>
            {mode === 'create' ? 'Create task' : `Edit ${task?.title}`}
          </CardTitle>
          ...
          <div className="flex items-start gap-3 rounded-xl border p-4 lg:col-span-2">
            <Checkbox
              ...
              onCheckedChange={(checked) =>
                form.setData('is_blocked', checked === true)
              }
            />
            <div className="space-y-1">
              <label className="text-sm font-medium" htmlFor="is_blocked">
                Task is blocked
              </label>
              <p className="text-sm text-muted-foreground">
                Blocked tasks make it easy to prototype queueing, escalations,
                or dependency views later.
              </p>
            </div>
          </div>
