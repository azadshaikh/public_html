...
export default function ReleasesIndex({ config, rows, filters, statistics, type }: any) {
    ...
    const columns: DatagridColumn<any>[] = [
        ...
        { key: 'version_type_label', header: 'Type', type: 'badge', sortable: true, sortKey: 'version_type', badgeVariantKey: 'version_type_badge' },
        { key: 'status_label', header: 'Status', type: 'badge', sortable: true, sortKey: 'status', badgeVariantKey: 'status_badge' },
        { key: 'release_at', header: 'Release Date', type: 'date', sortable: true },
    ...
    const bulkActions: DatagridBulkAction<any>[] = [
        ...(canDeleteReleases ? [{ key: 'bulk-delete', label: 'Move to Trash', icon: <Trash2Icon />, variant: 'destructive' as const, confirm: 'Move selected releases to trash?', onSelect: (items: any[], clear: () => void) => handleBulkAction('delete', items, clear) }] : []),
        ...(canRestoreReleases ? [{ key: 'bulk-restore', label: 'Restore', icon: <RefreshCwIcon />, confirm: 'Restore selected releases from trash?', onSelect: (items: any[], clear: () => void) => handleBulkAction('restore', items, clear) }] : []),
