...
export default function CmsPageForm({ mode, module, page, initialValues, options }: CmsPageFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>{mode === 'create' ? 'Create page' : `Edit ${page?.title}`}</CardTitle>
        ...
        <CardContent className="grid gap-6 lg:grid-cols-2">
          <div className="space-y-5 lg:col-span-2">
            ...
            <div className="space-y-2">
              ...
              <Textarea id="summary" value={form.data.summary} onChange={(event) => form.setData('summary', event.target.value)} rows={3} />
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium" htmlFor="published_at">Publish date</label>
            <Input id="published_at" type="date" value={form.data.published_at} onChange={(event) => form.setData('published_at', event.target.value)} />
            <InputError message={form.errors.published_at} />
          </div>

          <div className="flex items-start gap-3 rounded-xl border p-4 lg:col-span-2">
            <Checkbox id="is_featured" checked={form.data.is_featured} onCheckedChange={(checked) => form.setData('is_featured', checked === true)} />
            <div className="space-y-1">
              <label className="text-sm font-medium" htmlFor="is_featured">Feature on dashboard</label>
              <p className="text-sm text-muted-foreground">Use featured pages for hero banners, landing pages, or highlighted navigation blocks.</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 lg:col-span-2">
            ...
            <Button type="submit" disabled={form.processing}>
              {form.processing ? 'Saving...' : mode === 'create' ? 'Create page' : 'Save changes'}
            </Button>
          </div>
        </CardContent>
      </Card>
