      ...
      <section className="grid gap-4 xl:grid-cols-[1.35fr_1fr]">
        <Card className="border-none bg-gradient-to-br from-primary to-primary/80 text-primary-foreground shadow-none ring-0">
          <CardHeader className="space-y-4">
            <Badge variant="secondary" className="w-fit bg-primary-foreground/15 text-primary-foreground hover:bg-primary-foreground/15">
              Module runtime sample
            </Badge>
            <div className="space-y-2">
              <CardTitle className="text-3xl font-semibold tracking-tight md:text-4xl">
                {module.name} lives fully inside the module directory.
              </CardTitle>
              <CardDescription className="max-w-2xl text-primary-foreground/75">
                This page is rendered from the module, while reusing the host app layout, UI components,
          ...
          <CardContent className="grid gap-4 sm:grid-cols-3">
            <div>
              <div className="text-sm text-primary-foreground/70">Module slug</div>
            ...
            <div>
              ...
              <div className="mt-1 text-xl font-semibold">{modules.items.length} module(s)</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              The module runtime can register backend assets and frontend pages without moving code into the main app.
            </CardDescription>
          </CardHeader>
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Navigation can be declared from module config and exposed through the module.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Shared Inertia props can expose module metadata to any page.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {modules.items.map((managedModule) => (
              <div key={managedModule.slug} className="flex items-center gap-3 rounded-xl border p-4">
                <div className="rounded-lg bg-primary/10 p-2 text-primary">
                  <PackageIcon className="size-4" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="font-medium">{managedModule.name}</div>
                  <div className="text-sm text-muted-foreground">
                    {managedModule.slug} • {managedModule.url} • {managedModule.inertiaNamespace}
                  </div>
                </div>
                <Badge variant="outline">v{managedModule.version}</Badge>
              </div>
            ))}
          </CardContent>
        </Card>
