...
export default function ChatBotEdit({ module, prompt, initialValues, options }: ChatBotEditPageProps) {
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs} title={`Edit ${prompt.name}`} description={module.description}>
      <Head title={`Edit ${prompt.name}`} />
      <PromptTemplateForm mode="edit" module={module} prompt={prompt} initialValues={initialValues} options={options} />
    </AppLayout>
  )
}
