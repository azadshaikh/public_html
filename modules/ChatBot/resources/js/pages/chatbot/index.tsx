...
import { BotIcon, LayoutDashboardIcon, PencilIcon, PlusIcon, SearchIcon, SparklesIcon, Trash2Icon } from 'lucide-react'
...
export default function ChatBotIndex({ module, filters, prompts, stats, options, status }: ChatBotIndexPageProps) {
    ...
    >
      ...
      <section className="grid gap-4 xl:grid-cols-[1.35fr_1fr]">
        <Card className="border-none bg-gradient-to-br from-primary to-primary/80 text-primary-foreground shadow-none ring-0">
          <CardHeader>
            ...
            <CardTitle className="mt-4 text-3xl font-semibold tracking-tight md:text-4xl">
              Turn prompt ideas into reusable assistant building blocks.
            </CardTitle>
            <CardDescription className="text-primary-foreground/75">
              Prompt templates are a good starting point for assistants, support flows, escalation agents, and knowledge-powered responses.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-3">
            ...
            <div>
              <div className="text-sm text-primary-foreground/70">Active</div>
              <div className="mt-1 text-xl font-semibold">{stats.active}</div>
            </div>
            <div>
              <div className="text-sm text-primary-foreground/70">Defaults</div>
              <div className="mt-1 text-xl font-semibold">{stats.defaults}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Start simple, then grow into assistant orchestration.</CardDescription>
          </CardHeader>
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>Search prompt names or narrow by rollout status.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form action={chatbotIndexUrl} method="get" options={{ preserveScroll: true }} className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_auto]">
            <InputGroup className="w-full">
              <InputGroupAddon>
                <SearchIcon />
              </InputGroupAddon>
              <InputGroupInput name="search" defaultValue={filters.search} placeholder="Search name, slug, or purpose" />
            </InputGroup>
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-2">
        {prompts.data.length > 0 ? prompts.data.map((prompt) => (
          <Card key={prompt.id}>
            <CardHeader>
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  {prompt.is_default ? <Badge>Default</Badge> : null}
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
                ...
                <Badge variant="secondary">{prompt.tone}</Badge>
                <Badge variant="secondary">/{prompt.slug}</Badge>
              </div>
              <div className="flex flex-wrap justify-end gap-2">
                ...
                <Button size="sm" variant="destructive" onClick={() => handleDelete(prompt)}>
                  <Trash2Icon />
                  Delete
                </Button>
              </div>
            ...
            <CardContent className="p-6">
              <Empty>
                <EmptyHeader>
                  ...
                  <EmptyTitle>No prompt templates yet</EmptyTitle>
                  <EmptyDescription>Create a reusable system prompt to seed future assistants.</EmptyDescription>
                </EmptyHeader>
                <EmptyContent>
                  <Button asChild>
                    <Link href={chatbotCreateUrl}>
      ...
      {(prompts.prev_page_url || prompts.next_page_url) ? (
        <div className="mt-6 flex items-center justify-between gap-4 text-sm text-muted-foreground">
          <span>Showing {prompts.from ?? 0} to {prompts.to ?? 0} of {prompts.total} templates</span>
          <div className="flex gap-2">
            <Button asChild variant="outline" size="sm" disabled={!prompts.prev_page_url}>
              <Link href={prompts.prev_page_url ?? chatbotIndexUrl}>Previous</Link>
            </Button>
            <Button asChild variant="outline" size="sm" disabled={!prompts.next_page_url}>
              <Link href={prompts.next_page_url ?? chatbotIndexUrl}>Next</Link>
            </Button>
          </div>
        </div>
      ) : null}
