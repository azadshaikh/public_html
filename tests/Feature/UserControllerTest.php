...
class UserControllerTest extends TestCase
{
        ...
        $this->actingAs($this->admin)
            ...
            ->assertInertia(fn (Assert $page): Assert => $page
                ...
                ->where(
                    ...
                    '<p>Follow up on profile completion.</p>',
                )
    ...
    public function test_admin_can_store_sanitized_rich_text_note_content(): void
    {
        $user = User::factory()->create(['status' => Status::ACTIVE]);

        $this->actingAs($this->admin)
            ->postJson(route('app.notes.store'), [
                'content' => '<script>alert("xss")</script><h2>Important</h2><p><strong>Reach out</strong> tomorrow.</p>',
                'noteable_type' => User::class,
                'noteable_id' => $user->id,
                'visibility' => NoteVisibility::Team->value,
            ])
            ...
            ->assertJsonPath('notes.0.content_html', '<h2>Important</h2><p><strong>Reach out</strong> tomorrow.</p>');

        $storedNote = Note::query()->where('noteable_id', $user->id)->firstOrFail();

        $this->assertStringNotContainsString('<script>', $storedNote->content);
        $this->assertSame(
            '<h2>Important</h2><p><strong>Reach out</strong> tomorrow.</p>',
            $storedNote->content,
        );
    }
